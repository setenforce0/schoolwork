...
#+LATEX_HEADER: \usepackage[usenames,dvipsnames]{pstricks}
#+LATEX_HEADER: \usepackage{epsfig}
#+LATEX_HEADER: \usepackage{pst-grad}
#+LATEX_HEADER: \usepackage{pst-plot}
#+LATEX_HEADER: \usepackage[space]{grffile} 
#+LATEX_HEADER: \usepackage{etoolbox}
#+LATEX_HEADER: \makeatletter
#+LATEX_HEADER: \patchcmd\Gread@eps{\@inputcheck#1 }{\@inputcheck"#1"\relax}{}{}
...
*** Projectile Motion
**** A punter in a football game kicks a ball from the goal line at 60.0\deg from the horizontal at 25.0m/s
 
Convention: \uparrow \rightarrow +\\

 | X                 | Y                             |
 |-------------------+-------------------------------|
 | V_{1x} = 12.5 m/s | V_{1y} = 21.6 m/s             |
 | V_{2x} = 12.5 m/s | V_{2y} = 0 m/s(at max height) |
 | d_x = 55 m        | d_y = 23.8 m                  |
 | a_x = 0 m/s       | a_y = -9.8m/s[\downarrow]     |
 | t = 4.4 s         | t = 2.2 s                     |
***** Component Method
      |    X |    Y |
      |------+------|
 ...
 \(25 \sin 60 = Y\)
***** Solving using Big 5
 \noindent \(V_{2y}^2 = V_{1y}^2 + 2_{ay}d_y\)\\
 \(\frac{0 - 21.6^2}{2(-9.8)} = d\)\\
 \(d = 23.8\)

 \noindent \(V_{2y} = V_{1y} + a_yt\)\\
 \(t = \frac{0 - 21.6}{-9.8}\)\\
 \(t = 2.2 \) 
***** Using Symmetry
 \noindent \(t_x = 2t_y\)\\
...
**** An object is punted at 25.0m/s [40.0 \deg N of E] on G's home planet. What is the range of the object on level ground? (Use g = 18.0m/s^2)
 
 Convention: \uparrow \rightarrow +\\

  | X                 | Y                              |
  |-------------------+--------------------------------|
  | V_{1x} = 19.2 m/s | V_{1y} = 16 m/s                |
  | V_{2x} = 19.2 m/s | V_{2y} = 0 m/s (at max height) |
  | d_x = 34.6 m      | d_y = 7.1 m                    |
  | a_x = 0 m/s       | a_y = -18m/s[\downarrow]       |
  | t = 1.8 s         | t = 0.9 s                      |
***** Component Method
      |    X |  Y |
      |------+----|
...
**** An elastic loaded balloon launcher fires balloons at an angle of [38.0 \deg N of E] from the surface of the ground. If the initial velocity is 25.0 m/s, find how far away the balloons are from the launcher when they hit the level ground again.
 
 Convention: \uparrow \rightarrow +\\

  | X                 | Y                              |
  |-------------------+--------------------------------|
  | V_{1x} = 19.7 m/s | V_{1y} = 15.4 m/s              |
  | V_{2x} = 19.7 m/s | V_{2y} = 0 m/s (at max height) |
  | d_x = 63.04 m     | d_y = 12.1 m                   |
  | a_x = 0 m/s       | a_y = -9.8 m/s[\downarrow]     |
  | t = 3.2 s         | t = 1.6 s                      |
***** Component Method
      |    X |    Y |
      |------+------|
...
**** A football is kicked at 37.0 \deg to the horizontal at 20.0m/s from the player's hand at 1.00m from the ground. How far did the football travel before hitting the ground?

Convention: \uparrow \rightarrow +\\

 | X                 | Y                              |
 |-------------------+--------------------------------|
 | V_{1x} = 15.8 m/s | V_{1y} = 12.3 m/s              |
 | V_{2x} = 15.8 m/s | V_{2y} = 0 m/s (at max height) |
 | d_x = 37.92 m     | d_y = 7.7 m                    |
 | a_x = 0 m/s       | a_y = -9.8 m/s[\downarrow]     |
 | t = 2.4 s         | t = 1.2 s                      |
***** Component Method
      |    X |    Y |
...
1. The term /kinematics/ is best described as - c, the study of how objects move
2. Suppose you attach a string to the beginning of a winding path and walk to the end. There, you pull the string straight and measure its length. What would you be measuring? - a, displacement
3. You walk 27m[W] and 12m[E]. What is the total distance you have travelled? - b, 15m
4. Which of the following involves only a scalar? - b, A truck accelerates at 8m/s^2.
5. A cyclist travels 36km in 3.0h. What is her speed in metres per second? - d, 12m/s
6. For a straight line on a position-time graph, the rise refers to the change in which quantity? - d, position
7. The slope of a position-time graph measures which of the following quantities? - c, average quantity
8. For the position-time graph of an object moving in one dimension, which of the following properties would imply that the object has motion with non-uniform velocity?b, the graph is a curve.
9. An object accelerates at a rate of 1.2m/s^2[W] for 2.0s and has an initial velocity of 5.0m/s[E]. What is its final velocity? - d, 7.4m/s[W]
10. If you were to find the area under a velocity-time graph, which of the following quantities would you be calculating? a, displacement
11. Which of the following directions is equivalent to [E 31.7 /deg S]? - b, [S 58.3 /deg S]
12. How long would a distance of 625m be if represented on a scale diagram with a scale of 1 cm : 125 m? - a, 5cm
13. If you are standing next to a wall that runs east and west, and move a displacement of \delta \vec{d} = 25 m [E 63 /deg S], approximately how far away from the wall would you be?
14. 
*** Relative Velocity Pg 93-95 #37, 51
*** Adding Vector Quantities Pg 102, 105 #49, 46, 63, 67, Pg 92-95 #36, 43, 53
*** Projectile Motion Pg 102-105, 54, Pg 93 #38, 39, 41, 40, 52, 56, 57
*** Design and Perform Lab
** Chemistry
...
******* Hydrochloric Acid
``Hydrochloric acid is a colourless, highly pungent solution of hydrogen chloride (HCl) in water. It is a corrosive, strong mineral acid with many industrial uses. Hydrochloric acid is found naturally in gastric acid. When it reacts with an organic base it forms a hydrochloride salt.''
******* Zinc
...
\ce{C4H10(l) + 13/2O2(g) -> 4CO2(g) + 5H2O(g)}

\ce{C6H10(l) + 19/2O2 -> 6CO2 + 7H2O}

\ce{C6H14 + 8O2 -> 3CO + 3CO2 + 7H2O} 
\end{equation}

*** Acetylene Lab
<2016-11-02 Wed>

| Acetylene | Air | Flame Time | Flame Location           | Soot - amount, location |
|-----------+-----+------------+--------------------------+-------------------------|
|      100% |  0% | 5s         | Just at the top          | A lot, entire test tube |
|       75% | 25% | 4s         | Top and slightly down    | A lot, halfway down     |
|       50% | 50% | 3s         | Top and halfway          | Not much, 1/4 down      |
|       25% | 75% | 2s         | Top and all the way down | Barely any, 1/8 down    |
|       10% | 90% | 1s         | Top and all the way down | No noticeable soot      |
|        5% | 95% | 0.5s       | Top and all the way down | No noticeable soot      |

\ce{C2H2 + 5/2O2 -> 2CO2 + H20}

*** TODO P. 211 #1, 3-6
*** TODO P. 235 #1-18

   1. Which is the formula of the hydrocarbon shown in the chemical equation below?
      
   \ce{2CxHy(g) + 9O2(g) -> 6CO2(g) + 6H2O(g)}

   d) \ce{C3H6(g)} 
   
   2. If a car's exhaust system has a hole that allows exhaust gases to leak into the passenger compartment, the bigger danger when the car is running would be:
      
   b) Carbon monoxide

   3. Which would be present in carbonated pop?
      
   b) \ce{CO3(aq)}
   
   4. Which shows the general equation for a neutralization reaction that involves a hydroxide compound?
      
   c) acid + base -> water + ionic compound

   4. During the beginning of an ore, a detergent-like ionic compound is used to concentrate the part of the ore containing the metal. What is this process called?
...
*** Unit 3: Chemical Quantities
    ...
    1 mole = 6.02 \times 10^{23} entities (Avogadro's Number)

    602000000000000000000000

    1 mole is a case of chemical things.

    - Green Pea Analogy
      - Chemical entities are small.

    The mass of 1 mole of hydrogen atoms is called the molar mass, and is the mass number on the periodic table in grams.
    ...
    M_{H2O} H: 2 \cdot 1.0079 g / mol + 1 \cdot 15.9994 g / mol = 18.0152 g / mol 

    1 mol \ce{H2O} = 6.02 \times 10_{23} molecules = 18.0152 g / mol

    M_{\ce{Al2(SO4)3}} = 342.13588 g / mol

    M_{\ce{HC2H3O2}} = 
   
    What is the mass of 3 mol of \ce{HC2H3O2}? 
    ...
    \(= 3mol \cdot 60.052 g/mol\)

    \(= 180.156\)
*** Kadin's Formula Sheet
   1 mol = 6.02 \times 10^{23} entities
   m = nM
*** TODO p. 275 #1
...
Othello is married Desdemona, Iago and Roderigo. tell Brabantio and he accuses Othello of using witchcraft on his daughter, Iago leaves before Brabantio comes out of the house so that Othello doesn't know he doesn't like him. They go to session before the duke, Othello gives speech about how they came to love each other. Issues in Cyprus forces Othello to go to battle, and Desdemona wishes to go with him. Trusts his wife to Iago. Roderigo wants to kill himself but Iago convinces him not to, and to give him more money. 
**** Act II Scene I Summary
<2016-11-03 Thu>

Othello and company are caught in a storm at sea. Othello and Cassio get divided from each other due to the storm. Iago, Roderigo and Desdemona joke around trying to distract her from the fact that her husband is in danger from the storm, and Cassio's kindness to Desdemona plays perfectly into Iago's plan. Iago tells Roderigo to make Cassio angry to get him fired from his job as Lieutenant, so Iago can replace him. Turks drown at sea during storm, Othello and company have a party on Cyprus. 
**** Act II Scene III Summary
     <2016-11-06 Sun>

     Cassio starts his watch, Iago persuades Cassio to have some more liquor to drink. Iago then tells Roderigo to go after him, and Roderigo does what Iago says and pisses off Cassio. Cassio chases Roderigo and tries to kill him until Montano stops him, then Cassio starts fighting with Montano. Iago tells Roderigo to call a mutiny, and Othello wakes up and stops the fight. Iago says Cassio started it, and Othello tells Cassio that he is no longer one of his officers. Iago tells Cassio to appeal to Desdemona for his position back, and plans to use Desdemona's nature against her and tell Othello that she wants him to be officer because she is attracted to him.
**** Othello Final Test Review
     A/ Matching                   10 mks\\
     B/ Fill in the Blanks & T/F    6 mks\\
     C/ Iago's Web                  6 mks\\
     D/ Quotation Identification    8 mks\\
     E/ Analyze Soliloquy           5 mks\\
          a) who & when\\
          b) plot, theme, character development, etc...\\
          c) literary devices and effect\\
     F/ Short Answer (2)           15 mks\\
...
***** TODO Cuckoos Nest Essay
      ...
      Prove that Cuckoos Nest is an allegory
